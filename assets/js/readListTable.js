			...
			$.get(csvFile, function(data) {
				...
				$.each(lines, function(lineCount, line) {
					if ((lineCount == 0) && (typeof(options.headers) != 'undefined')) {
						...
						tableHTML += '<thead class="' + options.theadClass + '"><tr class="' + options.trClass + '"><th></th>';
						$.each(headers, function(headerCount, header) {
							tableHTML += '<th align="left" class="' + options.thClass + '">' + header + '</th>';
						});
					...
					if ((lineCount == options.startLine) && (typeof(options.headers) == 'undefined')) {
						...
						if (items.length > 1) {
							...
							tableHTML += '<tr class="' + options.trClass + ' ' + oddOrEven + '"><td>' + printedLines + '</td>';
							$.each(items, function(itemCount, item) {
								if(item.indexOf('*') >= 0)
								{
								item = item.replace(/[\\*]/g, "");  
								tableHTML += '<td class="' + options.tdClass + ' exceptional" style="background-color:#B7C4E3;"><a target="_blank" href="http://www.amazon.com/gp/search?ie=UTF8&camp=1789&creative=9325&index=books&keywords=' + item + items[1] + '&linkCode=ur2&tag=stamad-20">' + item + '</a></td>';
								}
								else if(item.indexOf('@~') >= 0)
								{
								item = item.replace(/[\\@]/g, "");   								item = item.replace(/[\\~]/g, ""); 
								tableHTML += '<td class="' + options.tdClass + ' audiobook abandoned" style="color:#E7A892;text-decoration:underline;"><a target="_blank" href="http://www.amazon.com/gp/search?ie=UTF8&camp=1789&creative=9325&index=audiobooks&keywords=' + item + items[1] + '&linkCode=ur2&tag=stamad-20">' + item + '  ðŸ”ˆ</a></td>';
								}
								...
								{
								item = item.replace(/[\\^]/g, "");   
								tableHTML += '<td class="' + options.tdClass + ' reread" style="font-style:italic;"><a target="_blank" href="http://www.amazon.com/gp/search?ie=UTF8&camp=1789&creative=9325&index=books&keywords=' + item + items[1] + '&linkCode=ur2&tag=stamad-20">' + item + '</a></td>';
								}
								else if(item.indexOf('~') >= 0)
								{
								item = item.replace(/[\\~]/g, "");   
								tableHTML += '<td class="' + options.tdClass + ' abandoned" style="color:#E7A892;"><a target="_blank" href="http://www.amazon.com/gp/search?ie=UTF8&camp=1789&creative=9325&index=books&keywords=' + item + items[1] + '&linkCode=ur2&tag=stamad-20">' + item + '</a></td>';
								}
								else if(item.indexOf('@') >= 0)
								{
								item = item.replace(/[\\@]/g, "");   
								tableHTML += '<td class="' + options.tdClass + ' audiobook" style="text-decoration:underline;"><a target="_blank" href="http://www.amazon.com/gp/search?ie=UTF8&camp=1789&creative=9325&index=audiobooks&keywords=' + item + items[1] + '&linkCode=ur2&tag=stamad-20">' + item + '  ðŸ”ˆ</a></td>';
								}
								else
								{
								tableHTML += '<td class="' + options.tdClass + '"><a target="_blank" href="http://www.amazon.com/gp/search?ie=UTF8&camp=1789&creative=9325&index=books&keywords=' + item + items[1] + '&linkCode=ur2&tag=stamad-20">' + item + '</a></td>';
							}
							});
							tableHTML += '</tr>';
						}
